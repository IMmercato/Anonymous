...
class OfflineMessageManager private constructor(context: Context) {
    ...
    interface MessageDao {
        ...
        @Query("SELECT * FROM queued_messages WHERE recipientB32 = :b32 AND status = 'PENDING'")
        suspend fun getPendingForRecipient(b32: String): List<QueuedMessage>

        @Query("SELECT * FROM queued_messages WHERE status = 'PENDING' ORDER BY createdAt ASC")
        fun getAllPendingFlow(): Flow<List<QueuedMessage>>

        @Query("UPDATE queued_messages SET status = :status WHERE id = :id")
        suspend fun updateStatus(id: String, status: QueuedMessage.Status)

        @Query("UPDATE queued_messages SET deliveredAt = :timestamp WHERE id = :id")
        suspend fun updateDeliveredAt(id: String, timestamp: Long)
        ...
        @Query("UPDATE queued_messages SET status = 'PENDING', retryCount = 0, nextRetryAt = :nextRetryAt, errorMessage = NULL WHERE id = :id")
        suspend fun resetForRetry(id: String, nextRetryAt: Long)
    }
    ...
    suspend fun getPendingForRecipient(b32Address: String): List<QueuedMessage> {
        return messageDao.getPendingForRecipient(b32Address)
    }

    suspend fun cancelMessage(messageId: String) {
        messageDao.updateStatus(messageId, QueuedMessage.Status.CANCELLED)
    }

    fun getAllPendingFlow(): Flow<List<QueuedMessage>> {
        return messageDao.getAllPendingFlow()
    }
    ...
    private suspend fun attemptDelivery(queuedMessage: QueuedMessage) {
        try {
            ...
            if (result.isSuccess) {
                messageDao.updateStatus(queuedMessage.id, QueuedMessage.Status.DELIVERED)
                messageDao.updateDeliveredAt(queuedMessage.id, System.currentTimeMillis())
                Log.i(TAG, "Successfully delivered queued message ${queuedMessage.id}")
            } else {
                handleRetryFailure(queuedMessage, result.exceptionOrNull()?.message ?: "Send failed")
            }
        } catch (e: Exception) {
            Log.e(TAG, "Delivery attempt failed for ${queuedMessage.id}", e)
            handleRetryFailure(queuedMessage, e.message ?: "Unknown error")
        }
    }
    ...
    private suspend fun handleRetryFailure(message: QueuedMessage, reason: String) {
        ...
        if (newRetryCount >= MAX_RETRIES) {
            ...
            val delayIndex = minOf(newRetryCount, RETRY_DELAYS.size - 1)
            val nextRetryDelay = RETRY_DELAYS[delayIndex]
    ...
    /**
     * Retry specific failed message manually
     */
    suspend fun retryMessage(messageId: String): Boolean {
        val message = messageDao.getById(messageId) ?: return false
        if (message.status != QueuedMessage.Status.FAILED && message.status != QueuedMessage.Status.CANCELLED) {
            return false
        }

        // Reset to pending
        messageDao.resetForRetry(messageId, System.currentTimeMillis())
        attemptDelivery(message.copy(retryCount = 0))
        return true
    }
